 ...
 */
function planDiff(rec, item, priorWorkSets, { coachAdded = false } = {}) {
  ...
  if (item.workSets > doneLast) {
    // «решение тренера» — только если план реально увеличен тренером;
    // иначе прошлый план был просто недовыполнен
    out.push({
      kind: 'sets', dir: 'up',
      text: coachAdded
        ? `подходы ${doneLast}→${item.workSets} · решение тренера`
        : `сегодня ${item.workSets} подх. (в прошлый раз сделан${doneLast === 1 ? '' : 'о'} ${doneLast})`,
    });
  ...
  function render() {
    ...
    for (const item of day.items) {
      ...
      if (locked) {
        ...
        const coachAdded = !!(hist && item.volAppliedAt === String(hist.date));
        const diffs = guide ? [] : planDiff(plan.rec, item, priorWork, { coachAdded });
