...
console.log('— calibrationGuide: пошаговые пояснения добора —');
const _item = { repRangeMin: 8, repRangeMax: 12 };
t('probe → шаг 1: прикидка, «не до отказа», правило посадки в диапазон', () => {
  const g = uiw.calibrationGuide({ mode: 'probe', rec: { weight: null, reps: 10 } }, _item, 2);
  assert(g && g.step === '1', JSON.stringify(g));
  assert(/прикид/i.test(g.title + g.text) && /не до отказа/i.test(g.text));
  assert(g.text.includes('8–12') && g.text.includes('RIR ≤ 3'), g.text);
});
t('ramp → шаг N+1, в тексте расчётный вес и правило посадки', () => {
  const g = uiw.calibrationGuide({ mode: 'ramp', calNo: 1, rec: { weight: 42.5, reps: 10, targetRIR: 2 } }, _item, 2);
  assert(g && g.step === '2', JSON.stringify(g));
  assert(g.text.includes('42.5 кг') && g.text.includes('8–12'), g.text);
  const g2 = uiw.calibrationGuide({ mode: 'ramp', calNo: 2, rec: { weight: 47.5 } }, _item, 2);
  assert(g2.step === '3');
});
t('обычная работа и «готово» → без пояснения', () => {
  assert(uiw.calibrationGuide({ mode: 'work' }, _item, 2) === null);
  assert(uiw.calibrationGuide({ mode: 'done' }, _item, 2) === null);
  assert(uiw.calibrationGuide(null, _item, 2) === null);
});
