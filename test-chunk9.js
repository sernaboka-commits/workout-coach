...
console.log('— runTarget: цель перед пробежкой —');
t('лёгкий: назначение + зона в bpm + ориентир прогрессии', () => {
  const last = R('easy', 8, 45, { avgHr: 130 });
  const tg = an.runTarget('easy', last, { hrMax: 190 });
  assert(tg.zoneLabel === 'Z1–Z3' && tg.zoneBpm && /аэробн/i.test(tg.goal), JSON.stringify(tg));
  assert(tg.lever === 'distance' && /8→8\.8/.test(tg.progress), tg.progress);
});
t('без прошлой пробежки → базовый ориентир', () => {
  const tg = an.runTarget('interval', null, { hrMax: 190 });
  assert(tg && /перв/i.test(tg.progress) && tg.zoneLabel === 'Z4–Z5', JSON.stringify(tg));
});
t('неизвестный тип → null', () => { assert(an.runTarget('zzz', null, {}) === null); });

console.log('— store.setRunPlanDay + monthGrid плановый бег —');
t('назначение/снятие типа на день недели', () => {
  let s = store.defaultState(ex.EXERCISE_LIBRARY);
  s = store.setRunPlanDay(s, 2, 'interval');   // среда — интервалы
  s = store.setRunPlanDay(s, 0, 'easy');        // понедельник — лёгкий
  assert(s.runPlan['2'] === 'interval' && s.runPlan['0'] === 'easy', JSON.stringify(s.runPlan));
  s = store.setRunPlanDay(s, 2, '');            // снять
  assert(s.runPlan['2'] === undefined);
});
t('monthGrid: плановый бег по дню недели попадает в ячейки', () => {
  let s = store.defaultState(ex.EXERCISE_LIBRARY);
  s = store.setRunPlanDay(s, 2, 'interval');    // среда
  const grid = an.monthGrid(s, { year: 2026, month: 6 });
  const wed = grid.flat().find((c) => c.date === '2026-07-08'); // среда
  const tue = grid.flat().find((c) => c.date === '2026-07-07'); // вторник
  assert(wed.plannedRun === 'interval' && tue.plannedRun === null, JSON.stringify({ wed: wed.plannedRun, tue: tue.plannedRun }));
});
